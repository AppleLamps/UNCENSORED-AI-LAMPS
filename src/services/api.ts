...
// Configuration
// Switched to OpenRouter unified endpoint (used for logs only; provider picks proxy when enabled)
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const USE_PROXY = (import.meta as any).env?.VITE_USE_PROXY === 'true';
const XAI_API_URL = USE_PROXY ? "/api/openrouter" : "https://openrouter.ai/api/v1/chat/completions";
...
const prepareApiKey = (apiKey: string): string => {
  // When proxying through serverless, API key is not required client-side
  if (!apiKey && USE_PROXY) return "";
  if (!apiKey) {
...
export const xaiService = {
  ...
  callAI: async (
    ...
    // Get API key from localStorage unless using proxy
