...
const SettingsPanel: React.FC<SettingsPanelProps> = ({
  ...
  const testApiKey = async () => {
    if (useProxy) {
      toast({ title: "Using server proxy", description: "Key test is not needed when proxy is enabled.", variant: "default" });
      return;
    }
    if (!inputApiKey.trim()) {
      toast({
    ...
    try {
      ...
      const response = await fetch(useProxy ? '/api/openrouter' : 'https://openrouter.ai/api/v1/chat/completions', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          ...(useProxy ? {} : { 'Authorization': `Bearer ${inputApiKey}` }),
